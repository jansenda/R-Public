---
title: "R Notebook - Template - MSSQL Data Access Using R (one table)"
date: "March 2023"
author: "Daan Jansen"
output:
  html_document:
...
### 2. Read the data from a SQL Server database and load table into tibble
...
# Read table from db
rs <- dbSendQuery(mydb, paste("select * from " ,db.table))

# Store table into dataframe
df  <-  dbFetch(rs, n = -1) # change name dataframe accordingly to your own choice

# disconnect
#on.exit(dbDisconnect(mydb)) # close database connection

# Clean up
rm(driver.name, server.name, server.port, db.name, db.table, user.name, user.pwd, rs)
